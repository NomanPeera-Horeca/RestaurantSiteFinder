...
import {
  ...
  Users, TrendingDown, Search, ShieldCheck
} from "lucide-react";
import {
  Table, TableBody, TableCell, TableHead, TableHeader, TableRow
} from "@/components/ui/table";
import type { FullReport, Competitor, WinningConcept, EquipmentBundle, MenuMarketFit } from "../../../shared/analysis-types";
    ...
    <Card className="border-border/50">
      <CardHeader className="pb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <CardTitle className="text-lg">Competitor Snapshot</CardTitle>
            <p className="text-sm text-muted-foreground">{competitors.length} restaurants within 1.5km</p>
          </div>
        </div>
      </CardHeader>
      <CardContent>
...
function ConceptsSection({ concepts, score, recommendation }: { concepts: WinningConcept[]; score: number; recommendation: string }) {
  return (
    <Card className="border-border/50">
      <CardHeader className="pb-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <CardTitle className="text-lg">Opportunity & Winning Concepts</CardTitle>
        ...
        {bundles.map((bundle, i) => (
          <div key={i} className="rounded-xl border border-border/50 bg-card overflow-hidden">
            <div className="bg-muted/30 p-4 border-b border-border/50 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
              ...
              <a href={HORECA.links.equipment} target="_blank" rel="noopener" className="text-xs text-primary hover:underline flex items-center gap-1 shrink-0">
...
export default function Report() {
  ...
  useEffect(() => {
    ...
    fullReport.mutate({ address, lat, lng, leadId });
  }, [address, lat, lng, leadId, navigate]);
      ...
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-center max-w-md px-6">
          ...
          <p className="text-muted-foreground mb-6">
            Our AI is analyzing competitors, reviews, and market data for <span className="font-medium text-foreground">{address}</span>
          </p>
          <div className="space-y-3">
            {[
