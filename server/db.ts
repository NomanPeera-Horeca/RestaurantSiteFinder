...
export async function updateLeadScore(id: number, score: number, recommendation: string) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  await db.update(leads).set({ opportunityScore: score, recommendation }).where(eq(leads.id, id));
}
